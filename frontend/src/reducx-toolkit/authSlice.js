...
export const forgotpassword=createAsyncThunk("/auth/forgotpassword",async(email)=>{
  try {
    ...
    toast.error(error?.message || "failed to sent mail please try again later")
  }
})
export const resetpassword=createAsyncThunk("/auth/resetpassword",async (formdata)=>{
  try {
    const res=await axios.post("http://localhost:4000/api/user/resetpassword",formdata,{
      withCredentials:true
    })
    ...
    toast.error(error?.message || "failed to reset passord please try again later")
  }
})
export const  updatepassword=createAsyncThunk("/auth/upadtepassword",async(formdata)=>{
  try {
    const res=await axios.post("http://localhost:4000/api/user/updatepassword",formdata,{
      withCredentials:true
    })
